...
echo ========================================
echo SPOTIFY PLUGIN BUILD SCRIPT
echo ========================================
echo.
...
if not exist plugin.py (
    echo [ERROR] plugin.py not found in current directory
    exit /b 1
)
echo [OK] plugin.py exists
...
python -c "import requests, spotipy" >nul 2>&1
if ERRORLEVEL 1 (
...
python -c "import plugin" >nul 2>&1
if ERRORLEVEL 1 (
...
python -m pyflakes plugin.py 2>_lint_errors.txt
...
del "%PROGRAMDATA%\NVIDIA Corporation\nvtopps\rise\plugins\spotify\spotify-plugin.log" >nul 2>&1
...
if %SMOKE_TEST_RESULT% NEQ 0 (
    ...
    if exist "!PLUGIN_LOG!" (
        ...
        echo ----------------------------------------
        powershell -Command "Get-Content '$env:PROGRAMDATA\NVIDIA Corporation\nvtopps\rise\plugins\spotify\spotify-plugin.log' | Where-Object { $_ -notmatch 'invalid JSON' -and ($_ -match 'ERROR' -or $_ -match 'CRITICAL' -or $_ -match 'EXCEPTION') } | Select-Object -Last 10"
        echo ----------------------------------------
    ...
    echo   - Syntax error in plugin.py
    ...
    echo HOW TO DEBUG
    echo ========================================
    echo   1. Run^: python plugin.py   (see full error^)
    echo   2. Check^: %%PROGRAMDATA%%\NVIDIA Corporation\nvtopps\rise\plugins\spotify\spotify-plugin.log
    echo   3. Verify^: pip list ^| findstr "spotipy requests"
    echo.
...
if exist %VENV%\Scripts\activate.bat (
	...
	if exist manifest.json (
		copy /y manifest.json "%SPOTIFY_DIR%\manifest.json" >nul
	...
	if exist config.json (
		copy /y config.json "%SPOTIFY_DIR%\config.json" >nul
		...
		echo [ACTION REQUIRED] Fill in config.json with your Spotify app credentials
		copy /y config.json "%SPOTIFY_DIR%\config.json" >nul
	)
	echo.

	echo [VALIDATION] Verifying build output...
	if not exist "%SPOTIFY_DIR%\g-assist-plugin-spotify.exe" (
	    echo [ERROR] Executable not found in output directory
	    call %VENV%\Scripts\deactivate.bat
	    exit /b 1
	)
	
	for %%A in ("%SPOTIFY_DIR%\g-assist-plugin-spotify.exe") do (
	    set EXE_SIZE=%%~zA
	)
	...
	echo ========================================
	echo.
	echo Output directory: %SPOTIFY_DIR%
	echo.
	echo Files created:
	dir /b "%SPOTIFY_DIR%"
	echo.
	echo Next steps:
	echo 1. Copy the entire '%SPOTIFY_DIR%' folder to:
	echo    %%PROGRAMDATA%%\NVIDIA Corporation\nvtopps\rise\plugins\
	echo 2. Configure credentials in config.json
